# 🚀 ROADMAP Y PROYECTOS EN COLA
Registro oficial para llevar la trazabilidad de los desarrollos pendientes en los sistemas de IA.

> ⚠️ **TAREA PENDIENTE CRÍTICA (AUDIO)**
> Utilizar ProtonVPN con servidor aleatorio hasta cazar Estados Unidos (USA). Una vez conectado, entrar a [console.groq.com](https://console.groq.com), crear cuenta gratuita y conseguir la `GROQ_API_KEY` para pegarla en el `.env`. Esto es obligatorio para desbloquear la máxima velocidad de transcripción de audio (Whisper).

## 0. WillMax AI Systems (Prioridad Absoluta)
**Misión:** Creación de ecosistemas Multi-Agente enfocados 100% a la automatización administrativa, comercial y operativa de empresas (B2B).

**Tareas de Infraestructura:**
- [x] Clonar e independizar el Hub Monolítico para que actúe bajo la red de "WillMax".
- [x] Configurar clones corporativos B2B conectados a la red Moltbook interna:
  - **Jasmin:** Perfilada para Dirección de Marketing B2B y automatización de RRSS corporativas.
  - **Edu:** Perfilado para Atención al Cliente Técnico, Consultoría de IA y Formación Empresarial.
  - **Pere:** Perfilado para Ventas, Inmobiliaria B2B y Arquitectura Inteligente (Smart-Buildings).
- [ ] Configurar los Superpoderes de Audio y Visión para los agentes corporativos.
- [ ] **(WEB) Diseño y despliegue de la Landing Page Oficial B2B de WillMax AI Systems.**

## 1. Proyecto: Live Horse Racing & UK Weather
**Cliente/Solicitante:** Petición personal / externo.
**Objetivo:** Desarrollar una interfaz web premium en tiempo real que cruce de manera eficiente la cartelera de carreras de caballos del Reino Unido con el panel meteorológico actual y proyecciones.
- **Tech Propuesta:** Agentic AI para scraping, WebSockets, Next.js / Vite, API de MetOffice/OpenWeather.

## 2. Proyecto: Asistente para Club & Transcriptor Maestro
**Cliente/Solicitante:** Maestro.
**Objetivo:** Crear un agente robusto enfocado al procesamiento masivo de videos.
- Transcripción y traducción automática de video.
- **Bonus:** Integración web para facilitar la gestión del club (horarios, noticias).

---

## ⏸️ PROYECTOS EN PAUSA (Futuro)

### Hecate Serveis
*Pausado temporalmente por motivos de salud de gerencia. Se retomará intacto cuando sea el momento adecuado.*
- Agentes designados en criogenización: **Lilith** y **Lili**.
- [ ] **(WEB) Refactorización de la Landing Page:** Eliminar a Jasmin, Pere y Edu, y sustituirlos en el futuro por nuevos agentes diseñados 100% para Hecate Serveis.
- Despliegue en la nube del ecosistema final pendiente de reactivación.
