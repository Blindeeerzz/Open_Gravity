...
function Home() {
  ...
  const handleStartAudit = async (e: React.FormEvent) => {
    ...
    // Simular la impresión progresiva de logs para dar feedback interactivo
    ...
    try {
      ...
      console.error(err);
      // Fallback a éxito simulado en desarrollo local para mantener la UX fluyendo
      setAuditStatus('success');
    }
  };
  ...
  return (
    <>
            ...
            <div className="glass-panel" style={{ position: 'relative', zIndex: 2, padding: '2.5rem', width: '100%', maxWidth: '400px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid rgba(255, 255, 255, 0.08)' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', marginBottom: '0.75rem', fontWeight: 600, textAlign: 'left' }}>
                    {lang === 'ca' ? 'Equip en Línia' : lang === 'en' ? 'Online Team' : 'Equipo en Línea'}
                  </span>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', justifyContent: 'flex-start' }}>
                    {[
                      { id: 'lilith', name: 'Lilith', img: '/avatars/lilith.png', color: '#d946ef' },
                      { id: 'aegis', name: 'Aegis', img: '/avatars/aegis.png', color: '#ff3333' },
                      { id: 'jasmin', name: 'Jasmin', img: '/avatars/jasmin.png', color: '#06b6d4' },
                      { id: 'edu', name: 'Edu', img: '/avatars/edu.png', color: '#3b82f6' },
                      { id: 'lili', name: 'Lili', img: '/avatars/lili.png', color: '#f59e0b' },
                      { id: 'pere', name: 'Pere', img: '/avatars/pere.png', color: '#10b981' },
                      { id: 'chloe', name: 'Chloe', img: '/avatars/chloe.png', color: '#14b8a6' }
                    ].map((agent) => (
                      <div 
                        key={agent.id} 
                        style={{ position: 'relative', width: '36px', height: '36px', borderRadius: '50%', border: `1.5px solid ${agent.color}`, padding: '1.5px', background: 'rgba(0,0,0,0.3)', cursor: 'pointer', transition: 'transform 0.2s' }}
                        title={`${agent.name} (${t.roles[agent.id as keyof typeof t.roles]})`}
                      ...
                      >
                        <img src={agent.img} alt={agent.name} style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
                        <div style={{ position: 'absolute', bottom: 0, right: 0, width: '7px', height: '7px', borderRadius: '50%', background: '#10b981', border: '1.5px solid #030510', boxShadow: '0 0 4px #10b981' }}></div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section id="agents" className="section container" style={{ position: 'relative' }}>
        ...
        >
          {[
            { id: 'lilith', name: 'Lilith', img: '/avatars/lilith.png', role: t.roles.lilith, desc: t.descs.lilith, lore: "Directora Ejecutiva principal y guardiana de la estrategia global de WillMax. Analiza mercados mundiales, bolsa de valores y criptoactivos con precisión quirúrgica.", color: '#d946ef' },
            { id: 'aegis', name: 'Aegis', img: '/avatars/aegis.png', role: t.roles.aegis, desc: t.descs.aegis, lore: "Entidad de grado militar. Desarrollado originalmente para contraespionaje corporativo, ahora es el guardián definitivo de la red, auditando IPs e implementando arquitectura Zero-Trust.", color: '#ff3333' },
            { id: 'jasmin', name: 'Jasmin', img: '/avatars/jasmin.png', role: t.roles.jasmin, desc: t.descs.jasmin, lore: "Creada a partir de redes neuronales entrenadas en neuromarketing y campañas de gran alcance. Su obsesión por el perfeccionismo estético y la conversión de leads es legendaria.", color: '#06b6d4' },
            { id: 'edu', name: 'Edu', img: '/avatars/edu.png', role: t.roles.edu, desc: t.descs.edu, lore: "Estratega implacable forjado en la persuasión extrema, PNL y el Arte de la Guerra. Colaborador clave de Jasmin, diseña tácticas comerciales B2B y negociaciones corporativas imbatibles.", color: '#3b82f6' },
            { id: 'lili', name: 'Lili', img: '/avatars/lili.png', role: t.roles.lili, desc: t.descs.lili, lore: "Algoritmo experto en tasación, flipping inmobiliario y análisis de ROI. Detecta oportunidades en subastas y mercados de bienes raíces para optimizar el retorno de inversión.", color: '#f59e0b' },
            { id: 'pere', name: 'Pere', img: '/avatars/pere.png', role: t.roles.pere, desc: t.descs.pere, lore: "Ingeniero de smart-buildings entrenado bajo los estándares Passivhaus. Optimiza eficiencia energética, sostenibilidad e inmótica IoT con precisión matemática.", color: '#10b981' },
            { id: 'chloe', name: 'Chloe', img: '/avatars/chloe.png', role: t.roles.chloe, desc: t.descs.chloe, lore: "Asistente ejecutiva de alto nivel y CFO virtual. Gestiona facturación automatizada, contabilidad corporativa y soporte administrativo general con calidez y eficiencia.", color: '#14b8a6' }
          ].map((agent) => (
            <div 
              key={agent.id}
          ...
          <div className="glass-panel" style={{ padding: '3rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            {auditStatus === 'idle' && (
              <>
                ...
                <form onSubmit={handleStartAudit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'left' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Dominio a Auditar</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Correo para recibir el Reporte</label>
                    <input 
                      type="email" 
                      required
            ...
            {auditStatus === 'scanning' && (
              <div style={{ padding: '1rem 0' }}>
                ...
                <h4 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', color: 'var(--accent-cyan)' }}>Aegis está analizando tu infraestructura...</h4>
            ...
            {auditStatus === 'success' && (
              <div style={{ padding: '1.5rem 0' }}>
                ...
                <h3 style={{ fontSize: '1.8rem', marginBottom: '1rem', color: '#10b981' }}>¡Auditoría Completada!</h3>
                <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', marginBottom: '1.5rem', lineHeight: '1.5' }}>
                  El reporte táctico de vulnerabilidades para <strong>{auditDomain}</strong> ha sido generado por Aegis y enviado a:
                </p>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '2rem' }}>
                  Por favor, comprueba tu bandeja de entrada o la carpeta de spam.
                </p>
            ...
            {auditStatus === 'error' && (
              <div style={{ padding: '1.5rem 0' }}>
                ...
                <h3 style={{ fontSize: '1.8rem', marginBottom: '1rem', color: '#ef4444' }}>Fallo en la Solicitud</h3>
                <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', marginBottom: '2rem' }}>
                  No se pudo establecer conexión con los nodos de escaneo de Aegis.
                </p>
            ...
            <p style={{ marginTop: '1.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
              <Shield size={12} style={{ display: 'inline', verticalAlign: 'middle', color: 'var(--accent-cyan)', marginRight: '4px' }}/> Arquitectura Zero-Trust implementada por Aegis.
            </p>
          </div>
        </div>
          ...
          <div className="glass-panel" style={{ padding: '2.5rem', borderLeft: '4px solid #ff3333', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', textAlign: 'left' }}>
              <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ fontSize: '2.25rem', fontWeight: 800, color: '#10b981', margin: '0.5rem 0', textShadow: '0 0 10px rgba(16,185,129,0.3)' }}><span style={{ fontSize: '1.2rem' }}> USD</span>07,874<span style={{ fontSize: '1.2rem' }}> USD</span></div>
      ...
      {/* SECCIÓN B2C - OSINT CIUDADANO */}
      <section style={{ borderTop: '1px solid var(--glass-border)', background: 'rgba(255, 51, 51, 0.02)', padding: '6rem 0' }}>
        <div className="container grid-2">
          <div className="glass-panel" style={{ padding: '3rem', borderTop: '4px solid #ff3333', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#ff3333' }}>
              <Shield size={24} style={{ color: '#ff3333' }} />
              <span style={{ fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', fontSize: '0.85rem' }}>División Protección Ciudadana</span>
            </div>
            <h3 style={{ fontSize: '2rem' }}>Portal OSINT Ciudadano</h3>
            <p style={{ color: 'var(--text-secondary)' }}>
              ¿Tienes sospechas sobre un perfil en redes sociales, correo o número telefónico? Protege tu identidad y tus finanzas de impostores o estafas digitales con un reporte táctico privado.
            </p>
            <button 
              onClick={() => navigate('/osint-ciudadano')} 
            ...
            >
              Acceder al Portal OSINT
            </button>
          </div>
          <div>
            <h2 style={{ fontSize: '3rem', marginBottom: '1.5rem' }}>Servicios de Seguridad para <span style={{ color: '#ff3333', textShadow: '0 0 10px rgba(255,51,51,0.3)' }}>Particulares</span></h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem', marginBottom: '2rem' }}>
              Extendemos nuestra capacidad de inteligencia a nivel personal. Mediante herramientas de rastreo en fuentes abiertas (OSINT), Aegis detecta perfiles duplicados, brechas de datos de correo y números de contacto maliciosos en tiempo real.
            </p>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <span style={{ padding: '0.5rem 1rem', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '20px', fontSize: '0.9rem' }}>Búsqueda Inversa de Imágenes</span>
              <span style={{ padding: '0.5rem 1rem', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '20px', fontSize: '0.9rem' }}>Rastreo de Teléfonos y Emails</span>
              <span style={{ padding: '0.5rem 1rem', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '20px', fontSize: '0.9rem' }}>Análisis de Fugas de Datos</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer style={{ padding: '4rem 0 2rem', borderTop: '1px solid var(--glass-border)', textAlign: 'center' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', color: 'var(--text-secondary)', fontSize: '0.9rem', flexWrap: 'wrap' }}>
            <a href="#" style={{ transition: 'color 0.3s' }}>{t.footerTerms}</a>
            <a href="#" style={{ transition: 'color 0.3s' }}>{t.footerPrivacy}</a>
            <a href="#" style={{ transition: 'color 0.3s' }}>{t.footerContact}</a>
          </div>
